...
RSpec.describe 'FunctionLogs' do
  describe 'GET /index' do
    context 'without a current user' do
      let(:account_with_user) { create(:account_with_user) }

      it 'redirects to login page' do
        get account_function_logs_path(account_with_user.account)
    ...
    context 'with multiple accounts' do
      ...
      let!(:function_log) { create(:function_log, twilio_account: account_with_user.twilio_accounts.first) }
      let!(:other_function_log) { create(:function_log, twilio_account: other_account_with_user.twilio_accounts.first) }
      ...
      it 'only shows function logs from first account' do
        get account_function_logs_path(account_with_user.account)
        expect(response.body).to include(function_log.sid)
          .and not_include(other_function_log.sid)
      end
    end
  end
end
